...
export default function RankingsLandingPage() {
  ...
  // Fetch toppers with infinite scroll logic
  const {
    data,
    fetchNextPage,
    hasNextPage,
    ...
    queryKey: ["global-toppers", debouncedSearch],
    queryFn: ({ pageParam = 1 }) =>
      leaderboardApi.getGlobalToppers(pageParam, 10, debouncedSearch),
  ...
  const allToppers = useMemo(() => {
    return data?.pages.flatMap((page) => page.data.toppers) || [];
  }, [data]);

  // Use all toppers directly (server-side filtering handled by queryKey/queryFn)
  const filteredToppers = allToppers;

  // Top 3 for spotlight (Only show from initial load when no search)
  const spotlight = debouncedSearch ? [] : allToppers.slice(0, 3);
  const remainingToppers = filteredToppers;

  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        <Loader2Icon className="h-10 w-10 text-blue-600 animate-spin" />
        <p className="text-slate-500 font-bold">Loading Hall of Fame...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4 text-center">
        ...
        <Button
          onClick={() => refetch()}
          className="bg-blue-600 font-bold h-12 px-8 rounded-xl"
        >
          Retry Loading
        </Button>
      </div>
      ...
      <section className="relative py-20 md:py-32 bg-linear-to-b from-slate-900 to-slate-800 text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-slate-400 mb-10 max-w-3xl mx-auto font-medium">
            Celebrate the hard work and success of our top performers. Compete
            with millions of aspirants and earn your place among the best.
          </p>
            ...
            {spotlight[1] && (
              <Card className="bg-white dark:bg-slate-900 border-0 shadow-2xl hover:-translate-y-2 transition-transform duration-500">
                <CardHeader className="text-center pb-2">
                  <div className="relative inline-block mx-auto">
                    <Avatar className="h-24 w-24 border-4 border-slate-100 dark:border-slate-800 shadow-xl">
                      <AvatarImage
                        src={
                          spotlight[1].avatar ||
                          `https://api.dicebear.com/7.x/avataaars/svg?seed=${spotlight[1].name}`
                        }
                      />
                  ...
                  <CardDescription className="font-semibold text-blue-600 truncate">
                    {spotlight[1].exam}
                  </CardDescription>
                </CardHeader>
            ...
            {spotlight[0] && (
              <Card className="bg-white dark:bg-slate-900 border-0 shadow-2xl hover:-translate-y-4 transition-transform duration-500 scale-105 md:scale-110">
                ...
                <CardHeader className="text-center pb-2">
                  <div className="relative inline-block mx-auto">
                    ...
                    <Avatar className="h-32 w-32 border-4 border-yellow-100 dark:border-yellow-900/30 shadow-2xl">
                      <AvatarImage
                        src={
                          spotlight[0].avatar ||
                          `https://api.dicebear.com/7.x/avataaars/svg?seed=${spotlight[0].name}`
                        }
                      />
                      <AvatarFallback>U</AvatarFallback>
                    </Avatar>
                  ...
                  <CardDescription className="font-bold text-yellow-600 truncate">
                    {spotlight[0].exam}
                  </CardDescription>
                </CardHeader>
                <CardContent className="text-center space-y-4">
                  ...
                  <div className="relative inline-block mx-auto">
                    <Avatar className="h-24 w-24 border-4 border-slate-100 dark:border-slate-800 shadow-xl">
                      <AvatarImage
                        src={
                          spotlight[2].avatar ||
                          `https://api.dicebear.com/7.x/avataaars/svg?seed=${spotlight[2].name}`
                        }
                      />
                      <AvatarFallback>U</AvatarFallback>
                    </Avatar>
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <div className="relative group flex-1 sm:flex-none">
              ...
              <Input
                ...
                className="pl-10 h-11 w-full sm:w-64 rounded-xl border-slate-200 dark:border-slate-800"
                value={search}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {remainingToppers.length > 0 ? (
                  remainingToppers.map((student) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <Badge
                          variant="outline"
                          className="rounded-lg font-bold text-[10px] px-2 py-0.5 border-slate-200 dark:border-slate-800 max-w-[150px] truncate"
                        >
                          {student.exam}
                        </Badge>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/profile/${student.userId}`}>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="opacity-0 group-hover:opacity-100 transition-opacity font-bold text-blue-600 hover:text-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20"
                          >
                            View Profile{" "}
          ...
          {hasNextPage && (
            <div className="p-6 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 text-center">
              <Button
                variant="outline"
                className="rounded-xl font-bold gap-2 h-12 px-8"
              ...
              >
                ...
                {isFetchingNextPage ? "Loading..." : "Load More Toppers"}
              </Button>
            </div>
          )}
        </div>
        ...
        <div className="bg-linear-to-r from-blue-600 to-indigo-700 rounded-[3rem] p-12 md:p-20 text-center text-white relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10">
            <h2 className="text-4xl md:text-6xl font-black mb-8 tracking-tighter">
              Ready to see your name <br /> on the leaderboard?
            </h2>
            <p className="text-xl text-blue-100 mb-10 max-w-2xl mx-auto">
              Start practicing today and climb the ranks. Every test you take
              brings you closer to being a champion.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              ...
              <Link href="/exams">
                <Button
                  variant="outline"
                  className="w-full sm:w-auto border-white/30 text-white hover:bg-white/10 h-14 px-10 rounded-2xl font-black text-lg backdrop-blur-md"
                >
                  Browse Exams
                </Button>
              </Link>
