...
export default function LeaderboardIndexPage() {
  ...
  const selectedGroup =
    groups.find((g) => g.seriesId === selectedId) ?? groups[0];
              ...
              <nav className="space-y-0.5">
                {groups.map((group, gi) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={cn("h-2 w-2 rounded-full shrink-0", col.dot)}
                      />
                  ...
                  >
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xl font-bold text-slate-900">
                        {selectedGroup.seriesName}
                      </p>
                      <p className="text-sm text-slate-500">
                        {selectedGroup.tests.length} test
                        {selectedGroup.tests.length !== 1 ? "s" : ""} — click
                        any to view rankings
                      </p>
                    </div>
                    <div
                      className={cn(
                        "h-11 w-11 rounded-xl flex items-center justify-center shrink-0",
                        a(selectedIndex).icon,
                      )}
                    >
      ...
      <div className="bg-white rounded-xl border border-slate-200 px-4 py-3.5 flex items-center gap-4 hover:border-slate-300 hover:shadow-sm transition-all">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-1">
            <MetaChip
              icon={<BarChart2Icon className="h-3 w-3" />}
              label={`${test.totalMarks} marks`}
            />
            {test.durationMins && (
              <MetaChip
                icon={<ClockIcon className="h-3 w-3" />}
                label={`${test.durationMins} min`}
              />
            )}
