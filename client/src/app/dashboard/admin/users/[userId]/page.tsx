...
export default function DeepUserProfilePage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  if (!profileData || !profileData.user) {
    return (
      <div className="text-center py-24 space-y-4">
        ...
        <Button onClick={() => router.push("/dashboard/admin/users")}>Return to Users</Button>
      </div>
    );
  }
  ...
  const roleIcon = user.role === "ADMIN" ? Shield : user.role === "INSTRUCTOR" ? UserIcon : UserIcon;
  ...
  const attemptColumns: ColumnDef<any>[] = [
    {
      accessorKey: "test",
      header: "Test Name",
      cell: ({ row }) => <span className="font-medium">{row.original.test?.title || "Unknown Test"}</span>,
    },
    {
      accessorKey: "status",
      header: "Status",
    ...
    },
    {
      accessorKey: "score",
      header: "Score",
      cell: ({ row }) => <span className="font-bold">{row.original.score}</span>,
    },
    {
      accessorKey: "accuracy",
      header: "Accuracy",
      cell: ({ row }) => <span>{row.original.accuracy ? `${row.original.accuracy}%` : "-"}</span>,
    },
    {
        ...
        <Card className="col-span-1 shadow-xs border-zinc-200 dark:border-zinc-800">
          ...
          <CardContent className="space-y-6 mt-4">
            ...
            <div className="pt-4 space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Access Controls</h4>
        ...
        <div className="col-span-1 md:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="shadow-xs bg-linear-to-br from-indigo-50 to-white dark:from-indigo-950/20 dark:to-zinc-950 border-indigo-100 dark:border-indigo-900/30">
              <CardContent className="p-4 flex flex-col gap-2">
                ...
                <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">{stats.totalAttempts}</div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-xs bg-linear-to-br from-emerald-50 to-white dark:from-emerald-950/20 dark:to-zinc-950 border-emerald-100 dark:border-emerald-900/30">
              <CardContent className="p-4 flex flex-col gap-2">
                ...
                <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">{Math.round(stats.avgScore)}</div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-xs bg-linear-to-br from-amber-50 to-white dark:from-amber-950/20 dark:to-zinc-950 border-amber-100 dark:border-amber-900/30">
              <CardContent className="p-4 flex flex-col gap-2">
                ...
                <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">{Math.round(stats.avgAccuracy)}%</div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-xs bg-linear-to-br from-violet-50 to-white dark:from-violet-950/20 dark:to-zinc-950 border-violet-100 dark:border-violet-900/30">
              <CardContent className="p-4 flex flex-col gap-2">
                ...
                <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">{Math.round((stats.avgTimeTaken) / 60)} <span className="text-base font-medium text-muted-foreground">min</span></div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="shadow-xs border-zinc-200 dark:border-zinc-800">
            <CardHeader>
              ...
              <CardDescription>The final submissions and grades from the last 10 attempts.</CardDescription>
            </CardHeader>
            <CardContent>
              <DataTable 
